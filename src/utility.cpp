...
namespace coffee_drv {
    ...
    void set_mem_monitor(unsigned int print_term) {
        Serial.println("[coffee_drv/utility][info] memory monitor configured");
    ...
    static void mem_monitor_task(void* task_param) {
        ...
        while (true) {
            ...
            Serial.printf("[coffee_drv/utility] mem_monitor: SRAM - total=%uKB, free=%uKB | PSRAM - total=%uKB, free=%uKB\n",
